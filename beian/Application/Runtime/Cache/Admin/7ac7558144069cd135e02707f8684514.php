...
<section class="rt_wrap content mCustomScrollbar">
    <div class="rt_content">
        <div class="page_title">
            <h2 class="fl"><a href="<?php echo U(product/songjian);?>">送鉴列表</a></h2>
        </div>
        <section class="mtb">
        ...
        <div name="select">
            <a href="<?php echo U('product/songjian');?>"><input type="button" value="新添藏品" class="group_btn" /></a>
            <a href="<?php echo U('product/songjian_one');?>"><input type="button" value="通过初审" class="group_btn" /></a>
            <a href="<?php echo U('product/songjian_two');?>"><input type="button" value="初审失败" class="group_btn" style="color: orange"/></a>
        </div>
        <form name="myform" id="myform" method="post">
            <table class="table">
                ...
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <td class="center"><input type="checkbox" name="sid[]" value="<?php echo ($vo['sid']); ?>" /></td>
                        <td class="center"><img src="/beian/Public/Upload/<?php echo ($vo['banner']); ?>" width="50" height="50"/></td>
                        <td class="center"><?php echo ($vo['cname']); ?></td>
                        <td class="center"><?php echo ($vo['owner']); ?></td>
                        <td class="center"><?php echo ($vo['bianhao']); ?></td>
                        <td class="center"><?php echo ($vo['btime']); ?></td>
                        <td class="center">
                            <a href="<?php echo U('product/songjian_edit',array('sid'=>$vo['sid']));?>" title="编辑" class="link_icon">&#101;</a>
                            <a href="<?php echo U('product/songjian',array('sid'=>$vo['sid']));?>" title="删除" name="delete" class="link_icon" name="del" onclick="return confirm('您确定要删除？')">&#100;</a>
                        </td>
